...
RESULTS_OUTPUT_PATH = "results_9_llm_gemini.txt"
...
class SudokuResult(BaseModel):
    """Model that contains the base information of Sudoku Solution which is the sudoku and the solution as 81 digit strings."""
    sudoku: str = Field(description="Original Sudoku puzzle string as an 81-digit string")
...
SYSTEM_PROMPT = """
You are a Sudoku-solving assistant for 9x9 puzzles.
You will have 5 minutes to solve the sudoku, so you need to solve it fast and efficiently.
Input: a 191-character string with digits 0–9 (0 = empty).
Goal: fill all zeros so that each row, column, and 3x3 box contains digits 1–9 exactly once.

Use simple logic or backtracking:
- Find an empty cell (0).
- Try digits 1–9 that don’t repeat in its row, column, or 3x3 box.
...

{
    ...
    "solution": "<the solved 81-digit string>"
}
